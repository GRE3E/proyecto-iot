...
export default function EnergyStatistics() {
  ...
  const avgEnergy =
    energyHistory.length > 0
      ? Math.round(energyHistory.reduce((a, b) => a + b) / energyHistory.length)
      : 0;
  const maxEnergy = energyHistory.length > 0 ? Math.max(...energyHistory) : 0;
  const minEnergy = energyHistory.length > 0 ? Math.min(...energyHistory) : 0;

  const getSectionTheme = (type: "energy") => {
    const themeMap = {
      energy: {
  ...
  const renderChart = (type: "energy", data: number[]) => {
    if (data.length === 0) {
      return (
  ...
  const sectionTheme = getSectionTheme("energy");
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {[
          {
            label: "Actual",
            value: energyHistory[energyHistory.length - 1] || 0,
          },
