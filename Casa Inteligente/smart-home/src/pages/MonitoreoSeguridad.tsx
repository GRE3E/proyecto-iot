...
const GaugeWithControl = ({
  ...
  return (
    <SimpleCard className={`p-6 bg-gradient-to-br ${bgGradient} backdrop-blur-sm border ${colors.cardBorder} rounded-2xl`}>
      <div className="flex items-center gap-3 mb-5">
        <div className={`p-2 ${colors.chipBg} rounded-lg`}>{icon}</div>
        <h4 className={`text-lg font-semibold ${colors.text}`}>{label}</h4>
      </div>
      <div className="relative w-40 h-40 mx-auto mb-6">
        <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
          <circle cx="50" cy="50" r={radius} fill="none" stroke={colors.sliderBg} strokeWidth={strokeWidth} className="opacity-50" />
          <circle
            cx="50"
            cy="50"
            r={radius}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <div className="flex items-baseline gap-1">
            ...
            <p className={`text-2xl font-bold ${colors.text}`}>{unit}</p>
          </div>
        </div>
      </div>
      <div className="px-2">
        <input
          ...
          className={`w-full h-2 ${colors.sliderBg} rounded-full appearance-none cursor-pointer ${colors.sliderAccent} slider-thumb`}
      ...
      <div className={`flex gap-1 border-b mt-4 ${colors.border}`} role="tablist">
        <button
          ...
          className={`px-6 py-3 font-medium flex items-center gap-2 relative transition-all ${
            activeTab === "seguridad" ? colors.text : `${colors.mutedText} ${colors.buttonHover}`
          }`}
        >
          ...
          {activeTab === "seguridad" && (
            <motion.span
              layoutId="underline"
              className={`absolute bottom-0 left-0 right-0 h-[3px] bg-gradient-to-r ${colors.secondary} rounded-full`}
            />
          )}
        </button>
        <button
          ...
          className={`px-6 py-3 font-medium flex items-center gap-2 relative transition-all ${
            activeTab === "monitoreo" ? colors.text : `${colors.mutedText} ${colors.buttonHover}`
          }`}
        >
      ...
      <div className="mt-6 px-6">
        <AnimatePresence mode="wait">
          ...
          >
            {activeTab === "monitoreo" ? (
              <>
                <h3 className={`text-xl md:text-2xl font-bold mb-6 ${colors.text} flex items-center gap-2`}>
                  <TrendingUp className={`${colors.cyanIcon} w-6 h-6`} />
                  Indicadores
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <GaugeWithControl
                    ...
                    maxValue={35}
                    label="Temperatura"
                    icon={<Thermometer className={`w-5 h-5 ${colors.cyanIcon}`} />}
                    unit="°C"
                    color="#22d3ee"
                    bgGradient={colors.cyanGradient}
                    colors={colors}
                  />
                  <GaugeWithControl
                    value={humidity}
                    ...
                    maxValue={80}
                    label="Humedad"
                    icon={<Droplets className={`w-5 h-5 ${colors.purpleIcon}`} />}
                    unit="%"
                    color="#a855f7"
                    bgGradient={colors.violetGradient}
                    colors={colors}
                  />
                </div>
              </>
              ...
              <>
                ...
                <SimpleCard
                  className={`p-5 sm:p-6 mb-6 border transition-all ${
                    cameraOn
                    ...
                    <div className="flex items-center gap-3">
                      <Video
                        className={`w-7 h-7 sm:w-8 sm:h-8 ${
                          cameraOn ? colors.cyanIcon : colors.mutedText
                        }`}
                      />
                      <div>
                        <p
                          className={`text-xs sm:text-sm ${
                            cameraOn ? colors.mutedText : colors.mutedText
                          }`}
                        >
                          Estado del Sistema
                        </p>
                        <p
                          className={`text-base sm:text-lg font-bold ${
                            cameraOn ? colors.greenText : colors.mutedText
                          }`}
                        >
