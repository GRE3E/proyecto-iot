...
async def register_speaker(name: str = Form(...), audio_file: UploadFile = File(...), is_owner: bool = Form(False)):
    ...
    try:
        ...
        async with get_db() as db:
            await utils._save_api_log("/speaker/register", {"name": name, "filename": audio_file.filename, "is_owner": is_owner}, response_data.dict(), db)
...
async def register_owner_speaker(name: str = Form(...), audio_file: UploadFile = File(...)):
    ...
    try:
        ...
        response_data = utils.get_module_status()
        async with get_db() as db:
            await utils._save_api_log("/speaker/register_owner", {"name": name, "filename": audio_file.filename}, response_data.dict(), db)
        return response_data
...
async def identify_speaker(audio_file: UploadFile = File(...)):
    ...
    try:
        ...
        async with get_db() as db:
            await utils._save_api_log("/speaker/identify", {"filename": audio_file.filename}, response_obj.dict(), db)
        return response_obj
...
async def update_speaker_owner(request: SpeakerUpdateOwnerRequest):
    ...
    try:
        async with get_db() as db:
            ...
            response_data = utils.get_module_status()
            await utils._save_api_log("/speaker/update_owner", request.dict(), response_data.dict(), db)
            return response_data
